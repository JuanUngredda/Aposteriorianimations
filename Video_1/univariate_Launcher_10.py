...
class PointsFromDistribution10(Scene):
    ...
    def construct(self):
        ...
        multimodal_normal_func_1 = lambda x: (prop) * get_normal_pdf(x=x, mu=5, variance=var1) * 70 * 0.2 + (
                1 - prop) * get_normal_pdf(x=x, mu=9, variance=var2) * 70 * 0.2
        multimodal_normal_func_1_density = x_axis.plot(multimodal_normal_func_1, color=BLUE)
        x_vals_normal_1 = get_normal_samples(mu=5, variance=1, n_samples=50000)
        x_vals_normal_2 = get_normal_samples(mu=9, variance=0.5, n_samples=50000)
        mean_2 = np.mean(x_vals_normal_1) * prop + np.mean(x_vals_normal_2) * (1 - prop)

        vertical_line_2 = Arrow(
            buff=0,
        ...
        self.add(x_axis, multimodal_normal_func_1_density, vertical_line_2)
        ...
        self.play(Transform(multimodal_normal_func_1_density, multimodal_normal_func_2_density),
                  Transform(vertical_line_2, vertical_line_3))
        self.wait()
...
if __name__ == '__main__':
    scene = PointsFromDistribution10()
