...
class PointsFromDistribution16(Scene):

    def construct(self):
        samples = []
        cov = np.array([[1, 0.8], [0.8, 1]])
        for i in range(400):
            sample = get_multivaraite_normal_samples(mu=np.ones(2) * 5, cov=cov, n_samples=1)
        ...
        group = self.create_contour_curves(xy_axis, cov)
        self.add(g1, group)
        self.wait()
        ...
        for i in range(400):
            sample = get_multivaraite_normal_samples(mu=np.ones(2) * 5, cov=cov2, n_samples=1)
            rand_point = Dot(xy_axis.coords_to_point(sample[0], sample[1]), color=RED)
            points_group_2.add(rand_point)

        group2 = self.create_contour_curves(xy_axis, cov2)
        self.play(FadeOut(group))
        self.play(Transform(points_group, points_group_2), Write(group2))
        self.wait()
...
if __name__ == '__main__':
    scene = PointsFromDistribution16()
